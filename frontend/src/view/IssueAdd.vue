...
import {getIdeas, getIdeasInterested, saveIdea, getTagByKeyword, getIdeaById} from '@/services/ideas.js'
...
const submitForm = async () => {
    ...
    if (valid) {
        let response = await saveIdea(idea_content.value, tag_selected_list.value, id.value)
...
onMounted(async ()=>{
    ...
    if(route.query.editid){
        id.value = route.query.editid

        let idea = await getIdeaById(route.query.editid)
        console.log(idea)

        idea_content.value = idea.description

        tag_option_list.value = idea.tags.map(tag => ({
            ...tag,
            label: tag.title
        }))
        tag_selected_list.value = idea.tags
        tag_value.value = idea.tags.map((tag) => tag.qid)
    }
})
