...
def viewUserByUsername(request, username):
    ...
    page_section = request.GET.get('section', 'entrepreneur')
    context = {
        'le_user': user,
    ...
    if page_section == 'entrepreneur':
        ...
        context['issues'] = pagina(request, issues)
    elif page_section == 'proponent':
        solutions = user.getTechSolutions()
        context['datalist'] = pagina(request, solutions)
    elif page_section == 'sponsor':
        context['datalist'] = pagina(request, user.getOffers())
    elif page_section == 'communicator':
        context['datalist'] = pagina(request, Media.objects.filter(createdByUser=user, deleted=False).order_by('-creationDate'))
    elif page_section == 'developer':
        context['datalist'] = pagina(request, user.getSolutions())
    else:
        raise Exception('param erro with page_section:'+page_section)

    return render(request, 'issues/user.html', context)


@login_required
def editUserForm(request):
    language_list = language_services.get_language_list()
    ...
    return render(request, 'issues/useredit.html', {
        'userinfo': userinfo,
...
def listUsers(request):
    ...
    return render(request, 'issues/userlist.html', {'users': users})
