from apps.issues.services import paypal_services, mail_services, FSException
# from emailmgr import utils as emailmgr_utils
# from emailmgr.models import EmailAddress
from apps.issues.models import *
from django.conf import settings
import re
from apps.issues.utils import bitcoin_validation
...
def edit_existing_user(user, request_dict):
    userinfo = user.getUserInfo()
    new_username = request_dict.get('username')
    ...
    if settings.BITCOIN_ENABLED:
        btcaddr = request_dict.get('bitcoin_receive_address')
    ...
    newEmail = request_dict['primaryEmail']
    newPaypalEmail = request_dict.get('paypalEmail')
    ...
    userinfo.save()
    #paypal_services.accepts_paypal_payments(user)  # TODO fixme: change paypal use
...
def add_reputation_by_new_issue(user):
